;(function( window, document ) {
	...
	FENDER.dashboard = {
		init: function() {
			...
			$(window).resize(function(){
				if($(window).width() > 768) {
					if(!$('.feed_list').hasClass('slick-initialized')) {
						_this.feedSlider();
					}
				} else if($('.feed_list').hasClass('slick-initialized')) {
		...
		rewardsHeight: function(){
			var rewardHeight = 0;

			$('.reward').removeClass('sized').css('height', 'auto');

			setTimeout(function(){
				$('.reward').each(function(){
					var thisHeight = $(this).outerHeight();

					if(thisHeight > rewardHeight) {
						rewardHeight = thisHeight;
					}
				});

				$('.reward').css('height', rewardHeight).addClass('sized');
			}, 500);
		},
		pushSidebar: function(){
			$('.latest_secondary').css('left', '0');

			var windowWidth = $(window).width(),
				sidebarWidth = $('.latest_secondary').width(),
				sidebarOffset = $('.latest_secondary').offset().left,
				sidebarPush = windowWidth - sidebarWidth - sidebarOffset;

			if(windowWidth > 1024) {
				$('.latest_secondary').css('left', sidebarPush);
			}
		}
	};
