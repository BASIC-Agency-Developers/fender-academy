...
class BNFW_Engine {
	...
	public function send_notification( $setting, $id ) {
		...
		if ( $notification_enabled ) {
			...
			$translated_posts_ids = $this->get_translated_posts_ids($setting['id']);
			$users_data = $this->get_users_emails($setting['users']);

			$data = array();
			for($i=0;$i<count($translated_posts_ids);$i++){		
				$data[$translated_posts_ids[$i]['language_code']] = $this->notifier->read_settings($translated_posts_ids[$i]['element_id']);		
			}
			$users_id = array();
			for($i=0;$i<count($setting['users']);$i++){
				$users_id[$i]=$users_data[$i]['ID'];
			}

			//get/set data for users without default language set/////
			$default_language_users_id=array_diff($setting['users'],$users_id);
			if(!empty($default_language_users_id)){
				$default_language_users = $this->get_default_language_users($default_language_users_id);
			}
			$users_data = array_merge($users_data,$default_language_users);
			///END get/set data for users without default language set////
			$emails  = $this->get_emails( $setting, $id );
			$headers = $this->get_headers( $emails );
			if(is_array($users_data)){
				for($i=0;$i<count($users_data);$i++) {

					$subject = $this->handle_shortcodes( $data[$users_data[$i]['value']]['subject'], $data[$users_data[$i]['value']]['notification'], $id );
					$message = $this->handle_shortcodes($data[$users_data[$i]['value']]['message'], $data[$users_data[$i]['value']]['notification'], $id );
					
					wp_mail( $users_data[$i]['user_email'], stripslashes( $subject ), $message, 'no-replay@fender.com');
				}
			}
		}
	}

	public function get_users_emails($users_ids){
		global $wpdb;
		$wpdb->show_errors();	

		$user_ids = implode(",", $users_ids);

		$results = $wpdb->get_results( "SELECT u.ID,u.user_email,u.display_name,xpd.value
									FROM ".$wpdb->prefix."users as u
									LEFT JOIN ".$wpdb->prefix."bp_xprofile_data as xpd
									ON u.ID=xpd.user_id 
									WHERE xpd.field_id = 2 AND u.ID IN (".$user_ids.")", ARRAY_A); //hardcode value
		return $results;
	}

	public function get_default_language_users($users_ids){
		global $wpdb;
		$wpdb->show_errors();	

		$user_ids = implode(",", $users_ids);

		$results = $wpdb->get_results( "SELECT u.ID,u.user_email,u.display_name
									FROM ".$wpdb->prefix."users as u
									
									WHERE u.ID IN (".$user_ids.")", ARRAY_A); 
	...
	public function get_translated_posts_ids($parent_translation_id){
		global $wpdb;
		$wpdb->show_errors();	

		$trid = $this->get_trid2($parent_translation_id);

		$results = $wpdb->get_results( "SELECT tr.element_id,tr.trid,tr.language_code
									FROM ".$wpdb->prefix."icl_translations as tr
									WHERE tr.trid = ".$trid->trid, ARRAY_A);

		return $results;
	}


	public function get_trid2($element_id){
		global $wpdb;
		$wpdb->show_errors();	

		$results = $wpdb->get_row( "SELECT trid FROM ".$wpdb->prefix."icl_translations WHERE element_id =".$element_id );
...
*/
	 ...
	 * @param int $id
	 */
	public function send_notification2( $setting, $id ) {
		/**
		 * BNFW - Whether notification is enabled?
		 *
		 * @since 1.3.6
