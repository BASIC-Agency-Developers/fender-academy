				...
				if ( empty( $options ) ) {
					...
					while ( isset( $_POST[$type . '_option'][$i] ) ) {
						...
						$options[] = (object) array(
							...
							'description'       => sanitize_text_field( stripslashes( $_POST[$type . '_value'][$i] ) ),
						);
				...
				if ( ! empty( $options ) ) {
					...
					for ( $i = 0, $count = count( $options ); $i < $count; ++$i ) :
						...
						<div id="<?php echo esc_attr( "{$type}_div{$j}" ); ?>" class="bp-option sortable">
							...
							<?php if ( $current_type_obj->supports_options ) : ?>
								<input type="text" name="<?php echo esc_attr( "{$type}_value[{$j}]" ); ?>" id="<?php echo esc_attr( "{$type}_value{$j}" ); ?>" value="<?php echo esc_attr( stripslashes( $options[$i]->description ) ); ?>" placeholder="language code: es,en,ro" />
							<?php endif; ?>
