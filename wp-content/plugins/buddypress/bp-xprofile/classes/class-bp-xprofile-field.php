	...
	public function save() {
		...
		if ( $wpdb->query( $sql ) !== null ) {
			...
			if ( $this->type_obj->supports_options ) {
				...
				$post_value  = ! empty( $_POST["{$this->type}_value"]           ) ? $_POST["{$this->type}_value"]           : '';				
				...
				$values       = apply_filters( 'xprofile_field_options_before_save', $post_value,  $this->type );
				...
				if ( !empty( $options ) ) {
					foreach ( (array) $options as $option_key => $option_value ) {
						...
						if ( '' != $option_value ) {
							$sql = $wpdb->prepare( "INSERT INTO {$bp->profile->table_name_fields} (group_id, parent_id, type, name, description, is_required, option_order, is_default_option) VALUES (%d, %d, 'option', %s, '%s', 0, %d, %d)", $this->group_id, $parent_id, $option_value, $values[$option_key], $counter, $is_default );
							if ( ! $wpdb->query( $sql ) ) {
								return false;
							}
						}
