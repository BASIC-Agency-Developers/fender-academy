...
if ( !class_exists( 'RWMB_Trivia_lesson_Quiz_Field' ) ) {
/**
 * Thim Theme
 *
 ...
 */
class RWMB_Trivia_lesson_Quiz_Field extends RWMB_Field {
/**
...
static function admin_enqueue_scripts() {
	...
	LPR_Admin_Assets::enqueue_script( 'thim-course', LearnPress()->plugin_url( 'inc/admin/meta-boxes/js/trivia.js' ), array( 'jquery', 'toastr', 'tojson' ) );
...
<div class="lpr-course-curriculum">
	...
	<ul class="lpr-curriculum-sections">
		<?php
		global $post;
		$course_sections = get_post_meta( $post->ID, '_lpr_trivia_lesson_quiz', true );           
		$section_state   = get_post_meta( $post->ID, '_lpr_trivia_section_state', true );
		...
		if ( $course_sections ): foreach ( $course_sections as $k => $section ): ?>
			...
			<li class="lpr-curriculum-section<?php echo $is_open ? "" : " closed"; ?>">
				<h3>
					...
					<span class="lpr-section-name-wrapper"><input name="_lpr_trivia_lesson_quiz[__SECTION__][name]" type="text" placeholder="<?php _e( 'Enter the section name and hit enter', 'learn_press' ); ?>" class="lpr-section-name" value="<?php echo esc_attr( $section['name'] ); ?>" /></span>
				</h3>
						...
						<?php   if ( isset( $section['lesson_quiz'] ) && is_array( $section['lesson_quiz'] ) ):
							...
							if ( $items = $wpdb->get_results( $query, OBJECT_K ) ):
                            	foreach ( $section['lesson_quiz'] as $id ):
									...
									$item = $items[$id];
									if ( 'lpr_triviaq' == $item->post_type ) $exclude_quiz[] = $item->ID;
									...
									<li class="lpr-<?php echo $item->post_type; ?>" data-id="<?php echo $item->ID; ?>" data-type="<?php echo $item->post_type; ?>">
										<?php if ( 'lpr_triviaq' == $item->post_type ) { ?>
										...
										<input type="hidden" name="_lpr_trivia_lesson_quiz[__SECTION__][lesson_quiz][]" value="<?php echo $item->ID; ?>" />
									</li>
								<?php endforeach; ?>
							<?php endif; ?>
						<?php endif; ?>
		...
		<li class="lpr-curriculum-section lpr-empty">
			<h3>
				...
				<!-- // actions -->
				<span class="lpr-section-icon"><i class="dashicons dashicons-pressthis"></i></span>
				<span class="lpr-section-name-wrapper"><input name="_lpr_trivia_lesson_quiz[__SECTION__][name]" type="text" placeholder="<?php _e( 'Enter the section name and hit enter', 'learn_press' ); ?>" class="lpr-section-name" /></span>
			</h3>
...
<script type="text/html" id="tmpl-curriculum-section">
	<li class="lpr-curriculum-section lpr-empty">
		<h3>
			...
			<!-- // actions -->
			<span class="lpr-section-icon"><i class="dashicons dashicons-pressthis"></i></span>
			<span class="lpr-section-name-wrapper"><input name="_lpr_trivia_lesson_quiz[__SECTION__][name]" type="text" placeholder="<?php _e( "Type section's name and hit enter", 'learn_press' ); ?>" class="lpr-section-name" /></span>
		</h3>
		...
		<# if( 'quiz' == data.type ){ #>
			...
			<# }else{ #>
				...
				<# } #>
					...
					<input type="hidden" name="_lpr_trivia_lesson_quiz[__SECTION__][lesson_quiz][]" value="{{data.id}}" />
	</li>
</script>
...
static function add_actions() {
	...
	add_action( 'wp_ajax_lpr_quick_add_t', array( __CLASS__, 'quick_add' ) ); 
	...
	add_action( 'wp_ajax_lpr_update_section_stateq', array( __CLASS__, 'update_section_state' ) ); 
...
static function remove_lesson_quiz() {
	...
	delete_post_meta( $lesson_quiz_id, '_lpr_trivia' );
	update_post_meta( $lesson_quiz_id, '_lpr_trivia', 0 );

}
static function update_section_stateq() {    
	$post_id = $_POST['post_id'];
	$section = $_POST['section'];
	update_post_meta( $post_id, '_lpr_trivia_section_state', $section );
	die();
}
...
static function quick_add() {
	...
	if ( $name && $type ) {
		$post_id = wp_insert_post(
			array(
				'post_title'  => $name,
				'post_type'   => $type == 'lesson' ? LPR_LESSON_CPT : 'lpr_triviaq',
				'post_status' => 'publish'
			)
		);
		if ( $post_id ) {
			...
			if ( $course_id ) {
				update_post_meta( $post_id, '_lpr_trivia', $course_id );
			}
		}
	}
	wp_send_json( $post );
...
static function update_course_curriculum() { 
	...
	$is_ajax = false;
	if ( !empty( $_REQUEST['action'] ) && 'lpr_update_trivia_curriculum' == $_REQUEST['action'] ) {
		$course_id = isset( $_POST['course_id'] ) ? intval( $_POST['course_id'] ) : 0;
		$is_ajax   = true; 
              
	} else {
		if ( 'lpr_trivia' != get_post_type() ) return;
		global $post;
		$course_id = $post->ID;
	}
	$meta_key = isset( $_POST['meta_key'] ) ? $_POST['meta_key'] : '_lpr_trivia_lesson_quiz';
	$metadata = isset( $_POST['_lpr_trivia_lesson_quiz'] ) ? $_POST['_lpr_trivia_lesson_quiz'] : false;
	if ( !$course_id ) {
		echo '__LPR_JSON__';
		wp_send_json(
			array(
	...
	$metadata = apply_filters( 'learn_press_trivia_curriculum', $metadata, $course_id );
	update_post_meta( $course_id, $meta_key, $metadata );
      
	do_action( 'learn_press_update_trivia_curriculum', $course_id, $metadata );
	...
	if ( $metadata ) foreach ( $metadata as $section ) {
		if ( !empty( $section['lesson_quiz'] ) && $lesson_quiz = $section['lesson_quiz'] ) {
			...
			foreach ( $lesson_quiz as $id ) {
				$query_values[] = $wpdb->prepare( "(%d, %s, %d)", $id, '_lpr_trivia', $course_id );
			}
