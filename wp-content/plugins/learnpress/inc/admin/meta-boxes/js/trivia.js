...
function sectionNameChange( evt ){
	...
	$('input.lpr-section-name', $li).attr('name', '_lpr_trivia_lesson_quiz[__SECTION__][name]');
    ...
    console.log($('li.lpr-curriculum-section').length);

    if( $('li.lpr-curriculum-section').length == 0){
...
function selectQuiz(e){
	if( !e.val ) return;
	var $item = $( wp.template('section-quiz-lesson')({title: e.object.text, id: e.val, type: 'lpr_triviaq'}) );
...
function ajaxAddQuiz( $sec, name ){
	$.post(ajaxurl, {action : 'lpr_quick_add_t', type: 'triviaq', name: name, course_id: course_id}, function(res){
        if( res ){
            res = res.split('__LPR_JSON__');
            res = res[res.length - 1];
            try{
		...
		if( res.ID ){
			var $item = $( wp.template('section-quiz-lesson')({title: res.post_title, id: res.ID, type: 'triviaq'}) ).hide();
			$sec.append($item);
			$item.fadeIn(750);
...
function updateSectionState(){
    ...
    var data = {
            action: 'lpr_update_section_stateq',
        ...
        .on('click', '.lpr-title', function(){
            ...
            function _update( apply ){
                ...
                $.post(ajaxurl, {id: id, action: 'lpr_quick_edit_lesson_quiz_name', name: newText}, function(res){
                    ...
                    $doc.unbind('click.hide_editable_quiz_lesson')
                    $input.remove();
                    toastr["success"](thim_course_localize.update_lesson_quiz.replace(/%s/, res.post_type == 'lpr_triviaq' ? 'Quiz' : 'Lesson') )
                }, 'text')
            }
