...
define( 'LPR_TRIVIAQ_SLUG', 'triviaq' );

if ( !class_exists( 'LPR_Triviaq_Post_Type' ) ) { 
	// class LPR_Quiz_Post_Type 
	final class LPR_Triviaq_Post_Type {
		...
		function __construct() {
			...
			add_filter( 'manage_lpr_triviaq_posts_columns', array( $this, 'columns_head' ) );
			add_action( 'manage_lpr_triviaq_posts_custom_column', array( $this, 'columns_content' ), 10, 2 );
			add_action( 'save_post_lpr_triviaq', array( $this, 'update_triviaq_meta' ) );
			add_filter( 'posts_fields', array( $this, 'posts_fields' ) );
			add_filter( 'posts_join_paged', array( $this, 'posts_join_paged' ) );
			add_filter( 'posts_where_paged', array( $this, 'posts_where_paged' ) );
			add_filter( 'posts_orderby', array( $this, 'posts_orderby' ) );
			add_filter( 'manage_edit-lpr_triviaq_sortable_columns', array( $this, 'columns_sortable' ) );
		...
		function register_post_type() {       
			register_post_type( 'lpr_triviaq',
				apply_filters( 'lpr__triviaq_post_type_args',
					array(
						'labels'             => array(
							'name'          => __( 'Trivia Quizzes', 'learn_press' ),
							'menu_name'     => __( 'Trivia Quizzes', 'learn_press' ),
							'singular_name' => __( 'Trivia Quizzes', 'learn_press' ),
							'add_new_item'  => __( 'Add New  Trivia Quizzes', 'learn_press' ),
							'edit_item'     => __( 'Edit  Trivia Quizzes', 'learn_press' ),
							'all_items'     => __( 'Trivia Quizzes', 'learn_press' ),
						),
						'public'             => true,
						...
						'hierarchical'       => true,
						'rewrite'            => array( 'slug' => _x( 'triviaq', 'Permalink Slug', 'learn_press' ), 'hierarchical' => true, 'with_front' => false )
					)
				)
			);
		}
		...
		function add_meta_boxes() {
			...
			$meta_box                                      = apply_filters(
				'learn_press_triviaq_question_meta_box_args',
				array(
					'title'      => __( 'Questions', 'learn_press' ),
					'post_types' => "lpr_triviaq",
					'id'         => 'questions',
					'fields'     => array(
						array(
							'name' => '',
							'desc' => '',
							'id'   => "{$prefix}triviaq_question",
							'type' => 'quiz_question'
						)
					)
				)
			);
			$GLOBALS['learn_press_triviaq_question_meta_box'] = new RW_Meta_Box( $meta_box ); 

			new RW_Meta_Box(
				array(
					'title'      => __( 'LearnPress Trivia Quiz Settings', 'learn_press' ),
					'post_types' => "lpr_triviaq",
					'context'    => 'normal',
					'priority'   => 'high',
					'fields'     => array(
						...
						array(
							'name' => __( 'Re-take trivia quiz', 'learn_press' ),
							'id'   => "{$prefix}retake_triviaq",
							'type' => 'number',
		...
		function enqueue_script() {
			if ( 'lpr_triviaq' != get_post_type() ) return;
			ob_start();
			?>
			<script>
				var form = $('#post');
				...
				form.submit(function (evt) {
					var $title = $('#title'),
						is_error = false;
					window.learn_press_before_update_triviaq_message = [];
					if (0 == $title.val().length) {
						window.learn_press_before_update_triviaq_message.push('<?php _e( 'Please enter the title of the trivia quiz', 'learn_press' );?>');
						$title.focus();
						is_error = true;
					}
					...
					if (window.learn_press_before_update_triviaq_message.length /*true == is_error*/) {
						if (window.learn_press_before_update_triviaq_message.length) {
							alert("Error: \n" + window.learn_press_before_update_triviaq_message.join("\n\n"))
						}
						evt.preventDefault();
						return false;
					}
			...
			<script type="text/html" id="tmpl-form-quick-add-question">
				<div id="lpr-form-quick-add-question" class="lpr-quick-add-form">
					<input type="text">
					<select class="lpr-question-types lpr-select2" name="lpr_question[type]" id="lpr-triviaq-question-type">
		...
		function columns_head( $columns ) {
			...
			if ( false !== $pos && !array_key_exists( 'lpr_trivia', $columns ) ) {
				$columns = array_merge(
					array_slice( $columns, 0, $pos + 1 ),
					array( 'lpr_trivia' => __( 'Trivia', 'learn_press' ), 'num_of_question' => __( 'Num. of questions', 'learn_press' ) ),
		...
		function columns_content( $name, $post_id ) {
			switch ( $name ) {
				case 'lpr_trivia':
					$course_id = get_post_meta( $post_id, '_lpr_trivia', true );
		 ...
		 * @param $quiz_id
		 */
		function update_triviaq_meta( $quiz_id ) {
			$course_id = get_post_meta( $quiz_id, '_lpr_trivia', true );
			if ( !$course_id ) {
				delete_post_meta( $quiz_id, '_lpr_trivia' );
				update_post_meta( $quiz_id, '_lpr_trivia', 0 );
			}
		}
		...
		function posts_where_paged( $where ) {
			...
			$where .= " AND (
                {$wpdb->postmeta}.meta_key='_lpr_trivia'
            )";
			if ( isset ( $_GET['meta_trivia'] ) ) {

				$where .= " AND (
                	{$wpdb->postmeta}.meta_value=" . intval( $_GET['meta_trivia'] ) . ")";
			}
			if ( isset( $_GET['s'] ) ) {
				$s = $_GET['s'];
				if ( empty( $s ) ) {
		...
		function columns_sortable( $columns ) {
			$columns['lpr_trivia'] = 'course';
			return $columns;
		}
	} // end LPR_Quiz_Post_Type
}
new LPR_Triviaq_Post_Type();
