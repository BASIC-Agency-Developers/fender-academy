 ...
 * @return  boolean
 */
function learn_press_user_has_completed_quiz( $user_id = null, $quiz_id = null, $quiz_type = 'lpr_quiz' ) {
	$completed = false;
	...
	// if $quiz_id is not passed, try to get it from current quiz
        ...
        if($quiz_type = 'lpr_quiz'){
            ...
            $quiz_id = learn_press_get_quiz_id( $quiz_id);
            $quiz_completed = get_user_meta( $user_id, '_lpr_triviaq_completed', true );
            $retake         = get_user_meta( $user_id, '_lpr_triviaq_retake', true );
        }
...
function learn_press_user_can_retake_quiz( $quiz_id = null, $user_id = null ) {
	...
	$available = 1;  //overwriting for fender purposes
	...
	$can_retake = true; //overwriting for fender purposes - default setting	

	//checking if there have been 30 days between last completion
	$query = $wpdb->prepare( "
        SELECT meta_value
        FROM {$wpdb->usermeta}
        WHERE user_id = %d
        AND meta_key = %s
    ", $user_id, '_lpr_quiz_completed');
	$quiz_last = $wpdb->get_var( $query );
	$quiz_last = unserialize($quiz_last);
	$last_taken = $quiz_last[$quiz_id];
	$now = time();
	...
	//end of checking if there have been 30 days between last completion
...
function learn_press_user_can_view_quiz( $quiz_id = null, $user_id = null ) {
	...
	if ( !$quiz_id ) {
        ...
        $weekly_trivia_id =  get_option( 'weekly_trivia', $default );
        $quizzes = learn_press_get_quizzes_trivia( $weekly_trivia_id);
        $quiz = $quizzes[0];
        $quiz = get_post($quiz);
        $return    = false;
if($quiz){ 
