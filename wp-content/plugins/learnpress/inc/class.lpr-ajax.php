...
if ( !class_exists( 'LPR_AJAX' ) ) {
	...
	class LPR_AJAX {
		...
		public static function finish_quiz() {
			...
			$passingCondition = learn_press_get_course_passing_condition($course_id);
			$result = learn_press_give_iq($quiz_id, $user_id );

			$final_quiz   = learn_press_get_final_quiz( $course_id );
			$final_result = learn_press_get_quiz_result( $user_id, $final_quiz );

			if($result >= $passingCondition){
				$user_points = get_user_meta( $user_id, '_lpr_iq_points' , true ); 
				$quizzPoints = get_post_meta( $course_id, '_lpr_iq_points' , true ); 
			
				if($user_points==""){
					$user_points = 0;
				}
				$newPoints = $user_points+$quizzPoints;
				update_user_meta( $user_id, '_lpr_iq_points', $newPoints);
			}
